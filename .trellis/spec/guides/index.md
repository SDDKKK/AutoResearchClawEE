...
| [External Search Strategy Guide](./search-guide.md) | Unified four-layer routing for external search (Context7, Perplexity, Grok Scripts) | When needing external info (library docs, APIs, news, fact-checking, deep research) |
...
**Quick Rule**: Library docs → Context7 (Layer 0). Quick facts → perplexity_ask (Layer 1). Multi-source comparison → perplexity_search + web_fetch (Layer 2). Deep research → perplexity_research (Layer 3). URL content → web_fetch.py. Code search → Augment/Grep.
