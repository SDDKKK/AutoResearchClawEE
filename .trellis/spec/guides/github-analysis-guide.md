...
| **Community discussions/reviews** | `perplexity_search` | `mcp__perplexity__perplexity_search(query="<project> review experience")` |
| **Competitor comparison** | `perplexity_search` | `mcp__perplexity__perplexity_search(query="<project> vs alternatives compare")` |
| **Deep technical research** | `perplexity_research` | `mcp__perplexity__perplexity_research(messages=[...])` |
...
```
Step 1: perplexity_search to get URL list
...
|-----------------|---------------|
| Perplexity MCP unavailable | `web_search.py` for search + `web_fetch.py` for key pages |
| `web_search.py` unavailable | `perplexity_search` for search |
| `web_fetch.py` all tiers fail | `perplexity_search` for summary; inform user to provide full text manually |
| `web_map.py` unavailable | Skip docs site structure discovery, doesn't affect core analysis |
| Context7 library not indexed | `perplexity_ask` for library docs |

**High-risk domains** (WeChat, Zhihu, Xiaohongshu):
- Prioritize `perplexity_search` for content summaries
