...
def main() -> int:
    ...
    parser.add_argument(
        "--name", "-n", required=True, help="Task name (e.g., user-auth)"
    )
    parser.add_argument(
        "--type", "-t", required=True, help="Dev type: python|matlab|both"
    )
    parser.add_argument(
    ...
    # Validate dev type
    if dev_type not in ("python", "matlab", "both"):
        log_error(f"Invalid dev type: {dev_type} (must be: python, matlab, both)")
        return 1
