#!/usr/bin/env python3
"""Web Fetch — 5-tier markdown fetcher with anti-bot domain awareness.
...
Result Contract (--json mode, aligned with content-extract SKILL):
  {
    "ok": true/false,
    "source_url": "...",
    "engine": "content-negotiation|jina-reader|cloudflare-workers-ai|tavily|mineru",
    "tier": 1-5,
    "markdown": "...",
    "notes": ["..."]
  }
...
def fetch_result(url: str) -> dict:
                ...
                if "text/html" in ct:
                    ...
                    with urllib.request.urlopen(req2, timeout=30) as resp2:
                        ...
                        if results and results[0].get("format") == "markdown":
                            md = results[0]["data"]
                            if not _looks_blocked(md):
                                print("[tier3: cloudflare-workers-ai]", file=sys.stderr)
    ...
    if antibot:
        ...
        notes.append(
            "all 5 tiers failed (CF content-negotiation, Jina Reader, CF Workers AI, Tavily, MinerU)"
        )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        action="store_true",
        dest="json_mode",
        help="Output unified Result Contract as JSON",
    )
    ap.add_argument(
        "--max-chars",
        type=int,
