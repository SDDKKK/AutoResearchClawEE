...
5. **Too Large / Should Be Split**
   ...
   - Would require touching too many systems (>6 files across >3 modules)
   - Cannot be completed in a reasonable scope
   - Single phase would produce >500 lines of new code
   - >5 acceptance criteria that span unrelated concerns
...
### Codex Feasibility Check (Optional)

For complex requirements where feasibility is uncertain, get a second opinion via Codex:
...
python3 ~/.claude/skills/with-codex/scripts/codex_bridge.py \
  ...
  --PROMPT "Evaluate feasibility of this requirement: <description>"
```

Only use when your own analysis is insufficient. See `.trellis/spec/guides/codex-assist.md`.
...
### Scope Sanity Check (before proceeding)

After accepting, verify the plan won't exceed a single agent's context budget:

| Check | Threshold | Action if exceeded |
|-------|-----------|-------------------|
| Core files to modify | ≤ 4 per phase | Split into sub-tasks |
| Acceptance criteria | Each must be independently verifiable | Rewrite vague criteria |
| Total new code | ≤ ~500 lines per phase | Split into smaller phases |
| Module dependencies | Explicitly mapped | Add dependency notes to prd.md |

**Dependency mapping**: If Phase 2 depends on Phase 1's output, add to task.json:
```json
"next_action": [
  {"phase": 1, "action": "implement", "depends_on": []},
  {"phase": 2, "action": "implement", "depends_on": [1]},
  {"phase": 3, "action": "check"},
  {"phase": 4, "action": "create-pr"}
]
```

**Verify coverage**: Every Acceptance Criterion in prd.md must have a corresponding verify method:
- Python: `uv run pytest` test case, or `ruff check` rule
- MATLAB: `checkcode` or manual verification step
- If a criterion cannot be verified programmatically, mark it as `[manual]` in prd.md
