...
Delegates implementation to Codex CLI via wrapper agent.
Same context injection as "implement", but execution by Codex.
...
Hook will auto-inject same context as implement (specs + prd + memory).
Context is written to a temp file and passed via `--context-file`.
Wrapper agent calls codex_bridge.py with the temp file path.
...
Task(
  ...
  prompt: "Review code changes for scientific correctness, cross-layer consistency, performance, and data integrity. Fix issues yourself",
  model: "opus",
  run_in_background: true
)
```
...
- All spec files from review.jsonl
- Cross-layer check specs
- prd.md for understanding intent
