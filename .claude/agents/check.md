...
The loop will NOT stop until verification passes.
...
### Fallback Path: Completion Markers

If no verify commands are configured, the loop checks for completion markers in your output.
Markers are generated from `check.jsonl` — each entry's `reason` field becomes a `{REASON}_FINISH` marker.
...
You MUST output these markers after each check passes:
- `LINT_FINISH` — After `uv run ruff check .` passes
- `TYPECHECK_FINISH` — After `uv run ruff format --check .` passes

If check.jsonl has no reasons, output: `ALL_CHECKS_FINISH`
