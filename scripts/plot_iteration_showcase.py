...
ax1.set_title(
    "Case A: Continual Meta-Learning\nfor Few-Shot Adaptation",
    fontsize=13,
    fontweight="bold",
    pad=12,
)
...
for i, (x, y, c) in enumerate(zip(case2_iters, case2_alignment, case2_colors)):
    ax2.scatter(
        x, y, s=120, color=c, zorder=6, edgecolors="white", linewidths=1.5, marker="s"
    )

# Annotate
ax2.annotate(
    "No improvement\n(minor code fix)",
    xy=(1, case2_alignment[1]),
    xytext=(1.3, 30),
    fontsize=8,
    color=GRAY,
    fontstyle="italic",
...
ax2.set_title(
    "Case B: RLHF with Curriculum-Based\nReward Shaping for LLM Alignment",
    fontsize=13,
    fontweight="bold",
    pad=12,
)
...
legend_elements = [
    ...
    mpatches.Patch(
        facecolor=RED, edgecolor="white", label="Regressed (auto-recovered)"
    ),
