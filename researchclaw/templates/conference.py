"""Conference template definitions for NeurIPS, ICLR, ICML, and IEEE Transactions.
...
Style files (``.sty``, ``.cls``) are bundled for IEEE templates — the generated
``.tex`` references them directly. For ML conferences, users download the official
files from the conference website. Download URLs are included as comments in the output.
"""
...
class ConferenceTemplate:
    ...
    author_format: str  # "neurips" | "iclr" | "icml" | "ieee"
    bib_style: str
    columns: int  # 1 or 2
    ...
    def render_preamble(
        ...
        # For IEEEtran, style_package is empty and options go on documentclass
        ...
        # Handle IEEEtran-style where document class has options and no separate package
        if self.style_package:
            style_line = f"\\usepackage{options}{{{self.style_package}}}"
            doc_class = f"\\documentclass{{{self.document_class}}}"
        else:
            style_line = ""
            doc_class = f"\\documentclass{options}{{{self.document_class}}}"
...
IEEE_TRANSACTIONS = ConferenceTemplate(
    name="ieee_transactions",
    display_name="IEEE Transactions",
    year=2025,
    document_class="IEEEtran",
    style_package="",  # IEEEtran is the document class, not a package
    style_options="lettersize,journal",
    extra_packages=(
        "amsmath",
        "amsfonts",
        "algorithmic",
    ...
    bib_style="IEEEtran",
    columns=2,
    style_download_url="https://www.ieee.org/content/dam/ieee-org/ieee/web/org/pubs/conference-latex-template.zip",
    preamble_extra="\\hyphenation{op-tical net-works semi-conduc-tor IEEE-Xplore}",
)

IEEE_TPWRS = ConferenceTemplate(
    name="ieee_tpwrs",
...
IEEE_CONFERENCE = ConferenceTemplate(
    name="ieee_conference",
    display_name="IEEE Conference",
    year=2025,
    document_class="IEEEtran",
    style_package="",
    style_options="conference",
    extra_packages=(
        "amsmath",
        "amsfonts",
        "algorithmic",
...
# -- Generic template (no special style files needed) --
...
CONFERENCE_REGISTRY: dict[str, ConferenceTemplate] = {
    ...
    "iclr": ICLR_2026,
    "icml": ICML_2026,
    "ieee_transactions": IEEE_TRANSACTIONS,
    "ieee_tpwrs": IEEE_TPWRS,
    "ieee_conference": IEEE_CONFERENCE,
    # Short aliases for IEEE
