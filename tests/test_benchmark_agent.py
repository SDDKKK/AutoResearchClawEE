...
class TestBenchmarkKnowledge:
    ...
    def test_each_domain_has_benchmarks_and_baselines(self) -> None:
        ...
        for did, info in data["domains"].items():
            ...
            assert len(info["standard_benchmarks"]) > 0, (
                f"Domain {did} has 0 benchmarks"
            )
    ...
    def test_baseline_entries_have_required_fields(self) -> None:
        ...
        for did, info in data["domains"].items():
            for bl in info["common_baselines"]:
                ...
                assert "source" in bl, (
                    f"Baseline {bl.get('name')} in {did} missing source"
                )
                assert "paper" in bl, (
                    f"Baseline {bl.get('name')} in {did} missing paper"
                )
...
class TestSurveyor:
    ...
    def test_domain_matching_image_classification(self) -> None:
        ...
        domains = agent._match_domains("Image Classification with Contrastive Learning")
    ...
    def test_domain_matching_rl(self) -> None:
        ...
        agent = SurveyorAgent(FakeLLM(), enable_hf_search=False)
        domains = agent._match_domains("Reinforcement Learning for Continuous Control")
    ...
    def test_domain_matching_knowledge_distillation(self) -> None:
        ...
        agent = SurveyorAgent(FakeLLM(), enable_hf_search=False)
        domains = agent._match_domains("Knowledge Distillation with Feature Alignment")
                ...
                json.dumps(
                    {
                        ...
                        "baselines": [
                            {"name": "CustomBL", "source": "custom", "paper": "X"}
                        ],
                        "rationale": "test",
                    }
                )
...
class TestSelector:
    ...
    def test_ranking_prefers_knowledge_base(self, benchmarks: list[dict]) -> None:
        ...
        kb_indices = [
            i for i, b in enumerate(ranked) if b["origin"] == "knowledge_base"
        ]
        hf_indices = [
            i for i, b in enumerate(ranked) if b["origin"] == "huggingface_hub"
        ]
        if kb_indices and hf_indices:
            assert min(kb_indices) < min(hf_indices)

    def test_execute_selects_minimum(
...
class TestOrchestrator:
    ...
    def test_orchestrate_saves_artifacts(self, tmp_path: Path) -> None:
        ...
        responses = [
            ...
            "def get_baselines(): pass",
            json.dumps(
                {"passed": True, "issues": [], "suggestions": [], "severity": "none"}
            ),
        ]
